...
import { pause } from "../spotify-player/handlers";
...
function ApplePlayer({ lobby_id, playerStatus, queue }) {
  ...
  useEffect(async () => {
    ...
    let playSong = async () => {
      ...
      if (prepForFirstPlay) {
        await applePlayer.seekToTime(timeStamp);
        prepForFirstPlay = false;
      }
    ...
    if (playerStatus && queue.length > 0) {
      ...
      timeStamp = playerStatus.timestamp / 1000;
      if (!playerStatus.paused) {
        await playSong();
        setPlaying(true);
        await applePlayer.seekToTime(timeStamp);
      } else {
        prepForFirstPlay = true;
      }
    }
    //Still testing
